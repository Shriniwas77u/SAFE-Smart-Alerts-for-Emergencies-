...
namespace SAFE.Server.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetDashboardSummary()
        {
            ...
            var teamCount = await _context.ResponseTeams.CountAsync();
            ...
            var recentIncidents = await _context.Incidents
                ...
                .Select(i => new {
                    ...
                    i.CreatedDate,
                    i.AssignedTeamId,
                    AssignedTeamName = i.AssignedTeamId != null ? _context.ResponseTeams.Where(t => t.ResponseTeamId == i.AssignedTeamId).Select(t => t.Name).FirstOrDefault() : null
                })
                .ToListAsync();

            var recentTeams = await _context.ResponseTeams
                .OrderByDescending(t => t.ResponseTeamId)
                .Take(5)
                .Select(t => new { t.ResponseTeamId, t.Name, t.EmergencyType, t.Region })
                .ToListAsync();
        ...
        public async Task<IActionResult> AssignTeam(int id, [FromBody] int teamId)
        {
            ...
            // Notify team (in-app notification)
            var notifTeam = new Notification
            {
                Type = "InApp",
                Message = $"You have been assigned to incident: {incident.Title} at {incident.Location}",
                IncidentId = incident.IncidentId,
                Status = "Pending"
            };
            _context.Notifications.Add(notifTeam);
            // Notify reporting user (in-app notification)
            var notifUser = new Notification
            {
                Type = "InApp",
                Message = $"Your reported incident '{incident.Title}' has been assigned to team {team.Name}",
                IncidentId = incident.IncidentId,
                UserId = incident.ReportedBy,
                Status = "Pending"
            };
            _context.Notifications.Add(notifUser);
            await _context.SaveChangesAsync();
            return Ok();
        }
    }
