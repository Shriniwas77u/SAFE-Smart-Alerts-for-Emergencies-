...
namespace SAFE.Server.Controllers
{
    ...
    public class IncidentsController : ControllerBase
    {
        ...
        public IncidentsController(SafeDbContext context, IConfiguration config)
        {
            _context = context;
            _config = config;
        }
        ...
        public async Task<ActionResult<IncidentDto>> CreateIncident([FromBody] CreateIncidentRequest request)
        {
            ...
            if (team != null)
            {
                ...
                // Send email (using SMTP config from appsettings)
                if (!string.IsNullOrEmpty(team.ContactEmail))
                {
                    try
                    {
                        var smtpSection = _config.GetSection("Smtp");
                        var mail = new MailMessage(smtpSection["From"], team.ContactEmail)
                        {
                        ...
                        using (var smtp = new SmtpClient(smtpSection["Host"], int.Parse(smtpSection["Port"])))
                        {
                            ...
                            smtp.Credentials = new NetworkCredential(smtpSection["User"], smtpSection["Password"]);
                ...
                if (!string.IsNullOrEmpty(team.ContactPhone))
                {
                    try
                    {
                        ...
                        using (var client = new HttpClient())
                        {
                            ...
                            {
                                new KeyValuePair<string, string>("To", team.ContactPhone),
                                new KeyValuePair<string, string>("From", fromNumber),
                                new KeyValuePair<string, string>("Body", $"Emergency: {incident.IncidentType} at {incident.Location}")
                            });
                            await client.PostAsync(twilioUrl, content);
                        }
                    }
                    catch { /* log error */ }
                }
